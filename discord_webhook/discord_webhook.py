...
class DiscordWebhook:
    ...
    def edit(self, sent_webhook):
        ...
        """
        sent_webhook = (
            sent_webhook if isinstance(sent_webhook, list) else [sent_webhook]
        )
        ...
        for i, webhook in enumerate(sent_webhook):
            ...
            if response.status_code in [200, 204]:
                logger.debug(f"[{i + 1}/{webhook_len}] Webhook edited")
            else:
                logger.error(
                    f"[{i + 1}/{webhook_len}] Webhook status code {response.status_code}: {response.content.decode('utf-8')}"
                )
            responses.append(response)
        return responses[0] if len(responses) == 1 else responses
    ...
    def delete(self, sent_webhook):
        ...
        for i, webhook in enumerate(sent_webhook):
            ...
            if response.status_code in [200, 204]:
                logger.debug(f"[{i + 1}/{webhook_len}] Webhook deleted")
            else:
                logger.error(
                    f"[{i + 1}/{webhook_len}] Webhook status code {response.status_code}: {response.content.decode('utf-8')}"
                )
...
class DiscordEmbed:
    ...
    def set_video(self, url=None, height=None, width=None):
        """
        set video of embed
